...
static void cmd_fifo_teardown(void) {
	if (cmd_fifo_source != NULL) {
		wl_event_source_remove(cmd_fifo_source);
		cmd_fifo_source = NULL;
	}
	if (cmd_fifo_fd >= 0) {
		close(cmd_fifo_fd);
		cmd_fifo_fd = -1;
...
static void cmd_fifo_process_line(struct tinywl_server *server, char *line) {
	...
	} else if (strcmp(line, "cycle_focus") == 0) {
		tinywl_server_cycle_focus(server);
	} else if (strcmp(line, "layout float") == 0) {
		tinywl_server_set_layout(server, LAYOUT_FLOAT);
	} else if (strcmp(line, "layout scroller") == 0) {
		tinywl_server_set_layout(server, LAYOUT_SCROLLER);
	} else if (strncmp(line, "scroll ", 7) == 0) {
		char *end = NULL;
		double d = strtod(line + 7, &end);
		if (end == line + 7) {
			d = 40.0;
		}
		tinywl_server_scroll_viewport(server, d);
	} else {
		wlr_log(WLR_ERROR, "unknown TINYWL_CMD_FIFO line: %s", line);
	}
}

static int cmd_fifo_readable(int fd, uint32_t mask, void *data) {
	...
	if (n < 0) {
		...
		wlr_log_errno(WLR_ERROR, "TINYWL_CMD_FIFO read");
		return 0;
	}
	if (n == 0) {
		return 0;
	}
	if (cmd_carry_len + (size_t)n >= sizeof(cmd_carry)) {
		wlr_log(WLR_ERROR, "TINYWL_CMD_FIFO buffer overflow");
		cmd_carry_len = 0;
		return 0;
	}
	memcpy(cmd_carry + cmd_carry_len, buf, (size_t)n);
	cmd_carry_len += (size_t)n;
	cmd_carry[cmd_carry_len] = '\0';

	char *p = cmd_carry;
	while (*p) {
		char *nl = strchr(p, '\n');
		if (!nl) {
			memmove(cmd_carry, p, strlen(p) + 1);
			cmd_carry_len = strlen(cmd_carry);
			return 0;
		}
		*nl = '\0';
		cmd_fifo_process_line(server, p);
		p = nl + 1;
	}
	cmd_carry_len = 0;
	return 0;
...
static void focus_toplevel(struct tinywl_toplevel *toplevel) {
	...
	if (toplevel->server->layout_mode == LAYOUT_SCROLLER) {
...
static void keyboard_handle_key(
	...
	if (event->state == WL_KEYBOARD_KEY_STATE_PRESSED) {
		for (int i = 0; i < nsyms; i++) {
			if (comp_ruleset_dispatch_key(&server->rules, server, modifiers, syms[i], true)) {
				handled = true;
				break;
			}
		}
...
static void server_cursor_axis(struct wl_listener *listener, void *data) {
	...
	if (comp_ruleset_dispatch_axis(&server->rules, server, mods,
...
void tinywl_server_scroll_viewport(struct tinywl_server *server, double delta) {
	server->scroll_viewport_offset += delta;
	if (server->scroll_viewport_offset < 0) {
...
int main(int argc, char *argv[]) {
	...
	const char *socket_export_path = NULL;
	bool parse_only = false;
	char config_path_buf[512];

	int c;
	while ((c = getopt(argc, argv, "s:c:pho:")) != -1) {
		switch (c) {
		case 's':
			startup_cmd = optarg;
			break;
		...
		case 'h':
		default:
			printf("Usage: %s [-c config.conf] [-o socket-name-file] [-p] [-s 'startup cmd']\n",
					argv[0]);
			return c == 'h' ? 0 : 1;
		}
	}
	...
	if (!config_path) {
		...
		if (xdg && xdg[0]) {
			snprintf(config_path_buf, sizeof config_path_buf, "%s/tinywl/config.conf", xdg);
		} else {
			const char *home = getenv("HOME");
			if (!home) {
				home = "";
			}
			snprintf(config_path_buf, sizeof config_path_buf, "%s/.config/tinywl/config.conf", home);
		}
		config_path = config_path_buf;
	}
	if (!comp_ruleset_load(&server.rules, config_path, parse_only)) {
		fprintf(stderr, "config errors in %s\n", config_path);
		return 1;
	}
	if (parse_only) {
		wlr_log(WLR_INFO, "config OK: %s", config_path);
		comp_ruleset_fini(&server.rules);
		return 0;
	}
	...
	if (socket_export_path != NULL) {
		FILE *sf = fopen(socket_export_path, "w");
		if (sf == NULL) {
			wlr_log_errno(WLR_ERROR, "failed to write -o %s", socket_export_path);
			wl_display_destroy(server.wl_display);
			wlr_backend_destroy(server.backend);
			return 1;
		}
		if (fprintf(sf, "%s", socket) < 0 || fflush(sf) != 0) {
			wlr_log_errno(WLR_ERROR, "failed to flush -o %s", socket_export_path);
			fclose(sf);
			wl_display_destroy(server.wl_display);
			wlr_backend_destroy(server.backend);
			return 1;
	...
	const char *cmdfifo = getenv("TINYWL_CMD_FIFO");
	if (cmdfifo != NULL && cmdfifo[0] != '\0') {
		cmd_fifo_fd = open(cmdfifo, O_RDONLY | O_NONBLOCK);
		if (cmd_fifo_fd < 0) {
			wlr_log_errno(WLR_ERROR, "open TINYWL_CMD_FIFO %s", cmdfifo);
		} else {
			struct wl_event_loop *loop =
				wl_display_get_event_loop(server.wl_display);
			cmd_fifo_source = wl_event_loop_add_fd(loop, cmd_fifo_fd,
					WL_EVENT_READABLE, cmd_fifo_readable, &server);
			if (cmd_fifo_source == NULL) {
				close(cmd_fifo_fd);
				cmd_fifo_fd = -1;
			} else {
				wlr_log(WLR_INFO, "TINYWL_CMD_FIFO=%s (automation)", cmdfifo);
			}
		}
	}
